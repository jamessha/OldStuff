...
void execute_one_inst(processor_t* p, int prompt, int print_regs) {
    ...
    switch (inst.rtype.opcode) /* could also use e.g. inst.itype.opcode */ {
        ...
        case 0xa: //opcode == 0xa (SLTI)
            p->R[inst.itype.rt] = ((int32_t) p->R[inst.itype.rs]) < inst.chunks16[0];
            p->pc += 4;
            break;
        ...
        case 0x24: // opcode == 0x24 (LBU)
            p->R[inst.itype.rt] = ((uint8_t) load_mem((p->R[inst.itype.rs]) + inst.chunks16[0], SIZE_BYTE));
            p->pc += 4;
            break;
            
        case 0x25: // opcode == 0x25 (LHU)
            p->R[inst.itype.rt] = ((uint16_t) load_mem((p->R[inst.itype.rs]) + inst.chunks16[0], SIZE_HALF_WORD));
            p->pc += 4;
            break;
