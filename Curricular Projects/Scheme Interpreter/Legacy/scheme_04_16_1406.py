...
class PrimitiveFunction(SchemeValue):
    ...
    def apply_step(self, args, evaluation):
    	if args.length() == 2:
    		arg1 = evaluation.full_eval(args.car)
    		arg2 = evaluation.full_eval(args.cdr.car)
    		evaluation.set_value(self.func(arg1, arg2))
    	elif args.length() == 1:
    		arg1 = evaluation.full_eval(args.car)
    		evaluation.set_value(self.func(arg1))
...
class LambdaFunction(SchemeValue):
    ...
    def apply_step(self, args, evaluation):
        #evaluation.env = self.env.make_call_frame(self.formals, args)
        ...
        if expr.symbolp():
        	for i in range(len(_PRIMITIVES)):
        		if str(expr) == _PRIMITIVES[i][0] or str(expr) in _PRIMITIVES[i][0]:
        			...
        			self.set_expr(PrimitiveFunction(value), self.env)		
