	...
	public static class Map1 extends
		...
		public void setup(Context context) {
			targetGram = context.getConfiguration().get("targetGram")
					.toLowerCase();
			String[] parts = targetGram.split(" ");
			gramLen = parts.length;
			try {
		...
		public void map(WritableComparable docID, Text docContents,
			...
			// Initiate first ngram
			LinkedList<String> nGram = new LinkedList<String>();
			int counter = 0;
			while (counter < gramLen && matcher.find()) {
				nGram.add(matcher.group());
				counter++;
			}
			
			// Stringifying the document
			String matchString = docContents.toString();
			
			// Collect all instances of the target gram
			ArrayList<Integer> targetIndexes = new ArrayList<Integer>();
			for (int i = -1; (i = matchString.indexOf(targetGram,
					i + 1)) != -1;) {
				targetIndexes.add(i);
			}
			...
			int currIndex = -1;
			parse: while (true) {
				// We find the next n-gram and find it's distance from the
				// target
				// gram
				String nextGram = "";
				for (String item : nGram) {
					nextGram += item.toLowerCase();
					nextGram += " ";
				}
				nextGram = nextGram.substring(0, nextGram.length() - 1);
				currIndex = matchString.indexOf(nextGram, currIndex + 1);
				// No self coocurrences
				if (!targetGram.equals(nextGram)) {
					// Finding the closest target gram
					double minSpaces = Double.MAX_VALUE;
					for (int target : targetIndexes) {
						...
						// Find number of spaces between
						double spaceCounter = 0;
						for (int i = start; (i = matchString.indexOf(" ",
								i + 1)) != -1 && i <= stop;) {
							spaceCounter ++;
							// Extended spaces should only count as 1
							while (matchString.charAt(i+1) == ' ')
							{
								i++;
							}
						}
					...
					if (targetIndexes.isEmpty()){
						minSpaces = Double.POSITIVE_INFINITY;
					}
					context.write(new Text(nextGram),
							new Text("" + func.f(minSpaces)));
				}
				if (!matcher.find()) {
					break;
				}
				// Delete the first word and add another to the rear for the
				// next n-gram
				nGram.poll();
				nGram.add(matcher.group());
			}
