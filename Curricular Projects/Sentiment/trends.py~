...
def find_centroid(polygon):
    ...
    while i < len(polygon)-1:
    	summation_centroid_x += ((latitude(polygon[i]) + latitude(polygon[i+1]))*(latitude(polygon[i])*longitude(polygon[i+1]) - latitude(polygon[i+1])*longitude(polygon[i])))
    	summation_centroid_y += ((longitude(polygon[i]) + longitude(polygon[i+1]))*(latitude(polygon[i])*longitude(polygon[i+1]) - latitude(polygon[i+1])*longitude(polygon[i])))
    	summation_area += (latitude(polygon[i])*longitude(polygon[i+1]) - latitude(polygon[i+1])*longitude(polygon[i]))
    	i += 1
...
def find_center(shapes):
    ...
    for x in centroid_list:
    	x_sum += latitude(x)*x[2]
    	y_sum += longitude(x)*x[2]
    	area_sum += x[2]
...
def find_closest_state(tweet, state_centers):
    ...
    'NJ'
    """
    position_tweet = make_position(latitude(tweet_location(tweet)), longitude(tweet_location(tweet)))
...
def calculate_average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state: 
    	...
    	if total_sent == None or (len(tweets_by_state[state]) - no_sent_tweet) == 0:
    		average_sent = None #marks the state for exclusion
    	else:
    	...
    	if state not in averaged_state_sentiments and average_sent != None:
