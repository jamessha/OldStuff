...
class PrimitiveFunction(SchemeValue):
    ...
    def apply_step(self, args, evaluation):
    	...
    	def listify(args):
    		if args != NULL:
    			arg_list.append(scm_eval(args.car))
    			return listify(args.cdr)
    	listify(args)
    	evaluation.set_value(self.func(*arg_list))
...
class LambdaFunction(SchemeValue):
    ...
    def apply_step(self, args, evaluation):
        def listify(args):
        	if args == NULL:
        		return []
        	return [args.car] + listify(args.cdr)
        def replacer(args, body, formals):
        	if body == NULL:
        		return NULL
        	elif type(body.car) == Pair:
        		return Pair(replacer(args, body.car, formals), replacer(args, body.cdr, formals))
        	for x in formals:
        		if x == body.car:
        			return Pair(args[formals.index(x)], replacer(args, body.cdr, formals))
        	return Pair(body.car, replacer(args, body.cdr, formals))
        formals_list = listify(self.formals)
        args_list = listify(args)
        func = replacer(args_list, self.body.car, formals_list)
        evaluation.env = evaluation.env.make_call_frame(self.formals, args)
        evaluation.set_expr(func)
...
class EnvironFrame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        def mapper(frame, formals, vals):
        	if formals == NULL:
        		return
        	elif formals.cdr == NULL:
        		frame.define(formals.car, vals)
        	else:
        		frame.define(formals.car, vals.car)
        		return mapper(frame, formals.cdr, vals.cdr)
        mapper(new_frame, formals, vals)
        return new_frame
...
class Evaluation:
    ...
    def step(self):
        ...
        expr = self.expr
        if expr.symbolp():
        	for i in range(len(_PRIMITIVES)):
        		if str(expr) == _PRIMITIVES[i][0] or str(expr) in _PRIMITIVES[i][0]:
        			value = _PRIMITIVES[i][1]
        			self.set_expr(PrimitiveFunction(value))		
        	if i == len(_PRIMITIVES)-1:
...
def scm_read():
    def read_tail():
        ...
        if syntax == SYMBOL:
        	...
        	val = 'quote'
        	first = Pair(Symbol.string_to_symbol(val), read_tail())
        	rest = NULL
        else:
        	return NULL
    ...
    if syntax == NUMERAL:
        ...
        val = 'quote'
        return Pair(Symbol.string_to_symbol(val), Pair(scm_read(), NULL))
    elif syntax == "(":
        return read_tail()
    else:
