...
public class Proj1 {
	...
	public static class Map1 extends
		...
		public void setup(Context context) {
			targetGram = context.getConfiguration().get("targetGram")
					.toLowerCase();
			// Transform the targetgram into a list for comparison and length
		...
		public void map(WritableComparable docID, Text docContents,
			...
			for (int begin = 0; begin + gramLen <= doc.size(); begin ++){
				...
				// emits the pait (gram, f(dist) 1), 1 denotes that this is the result of 1 gram (as opposed to multiple identical grams)
				context.write(new Text(newWord), new Text("" + func.f(minDist) + " " + 1));
			}
		}
	...
	public static class Combine1 extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			// Emits the key value pair (gram, Sg #occurrences)
	...
	public static class Reduce1 extends
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			// Turns all cooccurrences negative so hadoop will sort with descending magnitude
