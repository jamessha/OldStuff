...
class LambdaFunction(SchemeValue):
    ...
    def apply_step(self, args, evaluation):
        evaluation.set_expr(self.body.car, self.env.make_call_frame(self.formals, args)) #args should be evaluated in the call_frame
...
class EnvironFrame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        while formals.length() > 0:
        	new_frame.define(formals.car, scm_eval(vals.car))
        	if formals.cdr == NULL and vals.cdr != NULL:
        		new_frame.define(formals.car, vals)
        	formals = formals.cdr
        	vals = vals.cdr
        return new_frame
...
class Evaluation:
    ...
    def do_define_form(self):
        ...
        if target.symbolp():
            ...
            to_eval = Pair(Symbol.string_to_symbol("lambda"), Pair(target.cdr, Pair(self.expr.nth(2), NULL)))
