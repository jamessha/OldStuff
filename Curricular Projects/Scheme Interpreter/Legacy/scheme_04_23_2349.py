...
class PrimitiveFunction(SchemeValue):
    ...
    def apply_step(self, args, evaluation):
        args_list = []
        while args.length() > 0:
            args_list.append(evaluation.full_eval(args.car))
            args = args.cdr
        try:
    	    evaluation.set_value(self.func(*args_list))
        except TypeError:
            raise SchemeError("Incorrect number of arguments given")
...
class LambdaFunction(SchemeValue):
    ...
    def apply_step(self, args, evaluation):
        vals = []
        while args != NULL:
    	    vals.append(evaluation.full_eval(args.car))
    	    args = args.cdr
        evaluation.set_expr(self.body.car, self.env.make_call_frame(self.formals, vals)) #args should be evaluated in the call_frame
...
class EnvironFrame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        while type(formals) == Pair:
        	new_frame.define(formals.car, vals.pop(0))
        	formals = formals.cdr
        if formals != NULL:
        	def builder(l):
        		if len(l) == 0:
        			...
        			return Pair(l[0], builder(l[1:]))
        	new_vals = builder(vals) 
        	new_frame.define(formals, new_vals)
        return new_frame
...
class Evaluation:
    ...
    def step(self):
        ...
        expr = self.expr
        if expr.symbolp():
        	self.set_value(self.env.find(expr)[expr])
    ...
    def do_lambda_form(self):
        ...
        # LambdaFunction takes 4 arguments: self, formal parameter list, body, env
        formals = self.expr.cdr.car
        values = self.expr.cdr.cdr
        try:
        	self.expr.cdr.cdr = Pair(Pair(Symbol.string_to_symbol("begin"), self.expr.cdr.cdr), NULL)
        	values = self.expr.cdr.cdr
        except:
        	pass
        func = LambdaFunction(formals, values, self.env)
        self.set_value(func)

    def do_if_form(self):
        self.check_form(4, 4)
        cond = self.full_eval(self.expr.cdr.car)
        poss1 = self.expr.cdr.cdr.car
        poss2 = self.expr.cdr.cdr.cdr.car
        if cond:
        	self.set_expr(poss1)
        else:
        	self.set_expr(poss2)
    ...
    def do_and_form(self):
        ...
        if self.expr.length() == 1:
            self.set_value(TRUE)
            return
        conditional = self.expr.car
        rest = self.expr.cdr
        if rest.cdr == NULL:
        	self.set_expr(rest.car)
        elif not self.full_eval(rest.car):
        	self.set_value(FALSE)
        else:
        	self.set_expr(Pair(conditional, rest.cdr))
    ...
    def do_or_form(self):
        ...
        if self.expr.length() == 1:
            self.set_value(FALSE)
            return
        condition = self.expr.car
        rest = self.expr.cdr
        if rest.cdr == NULL:
        	self.set_expr(rest.car)
        elif self.full_eval(rest.car):
        	self.set_value(self.full_eval(rest.car))
        else:
        	self.set_expr(Pair(condition, rest.cdr))
        ...
        for i in range(1, num_clauses):
            ...
            if test:
                if clause.length() == 1:
                    ...
                    to_eval = Pair(clause.cdr.cdr.car, Pair(test, NULL))
                    self.set_expr(to_eval)
                else:
    ...
    def do_set_bang_form(self):
        self.check_form(3, 3)
        assert self.env.find(self.expr.nth(1)) #define catching actually already handled in the environment
        reciever = self.expr.nth(1)
        donor = self.expr.nth(2)
        self.env.find(reciever).define(reciever, self.full_eval(donor))
        self.set_value(UNSPEC)
    ...
    def do_define_form(self):
        ...
        if target.symbolp():
            ...
            new_target = target.car
            to_eval = Pair(Symbol.string_to_symbol("lambda"), Pair(target.cdr, self.expr.cdr.cdr))
            self.env.define(new_target, self.full_eval(to_eval))
            self.set_value(UNSPEC)
    ...
    def do_let_form(self):
        ...
        def symbol_builder(bind):
            if bind == NULL:
            	return NULL
            return Pair(bind.car.car, symbol_builder(bind.cdr))
        symbols = symbol_builder(bindings)
        vals_list = []
        while bindings != NULL:
            vals_list.append(self.full_eval(bindings.car.cdr.car))
            bindings = bindings.cdr
        let_frame = self.env.make_call_frame(symbols, vals_list)
    ...
    def do_let_star_form(self):
        ...
        if not scm_listp(bindings):
            raise SchemeError("bad bindings list in let form")
        symbols = bindings.car
        vals = bindings.cdr.car
        vals_list = []
        while vals.length() != 0:
            #vals_list.append(self.full_eval(vals.car))
            to_bind = self.full_eval(vals.car)
            let_frame = self.env.make_call_frame(Pair(symbols.car, NULL), [to_bind])
            symbols = symbols.cdr
            vals = vals.cdr
            self.set_expr(exprs, let_frame)
        for i in range(0, exprs.length()-1):
            self.full_eval(exprs.car, let_frame)
            exprs = exprs.cdr
        self.set_expr(exprs.car, let_frame)
    ...
    def do_case_form(self):
        self.check_form(2)
        def checker(target, l):
        	if l == NULL:
        		return False
        	elif target.eqvp(l.car):
        		return True
        	return checker(target, l.cdr)
        num_clauses = self.expr.length()
        target = self.full_eval(self.expr.nth(1))
        cont = False
        for i in range(2, num_clauses):
        	if self.expr.nth(i).car is self._ELSE_SYM:
        		cont = True
        		if i == 2:
        			raise SchemeError("badly formed case statement")
        		self.set_value(self.full_eval(self.expr.nth(i).cdr.car))
        		break
        	lis = self.full_eval(Pair(Symbol.string_to_symbol("quote"), Pair(self.expr.nth(i).car, NULL)))
        	if checker(target, lis):
        		cont = True
        		self.set_value(self.full_eval(self.expr.nth(i).cdr.car))
        		break
        if not cont:
        	raise SchemeError("no case found")
    ...
    def check_formals(formal_list):
        ...
        symbol_list = []
        while type(formal_list) == Pair:
        	if formal_list.car in symbol_list:
        		raise SchemeError("non-distinct formal parameter")
        	if type(formal_list.car) != Symbol:
        		raise SchemeError("formal parameter not symbol")
        	symbol_list.append(formal_list.car)
        	formal_list = formal_list.cdr
        if type(formal_list) != Symbol and formal_list != NULL:
        	raise SchemeError("formal parameter not symbol")	
...
def scm_read():
    def read_tail():
        ...
        input_port.pop()
        if syntax == SYMBOL:
        	first = Symbol.string_to_symbol(val)
        	rest = read_tail()
        elif syntax == NUMERAL:
        	first = Number(val)
        	rest = read_tail()
        	...
        	val = Symbol.string_to_symbol('quote')
        	first = Pair(val, Pair(scm_read(), NULL))
        	rest = read_tail()
        elif syntax == ".":
        	temp = scm_read()
        	input_port.pop()
    ...
    if syntax == NUMERAL:
        ...
        val = Symbol.string_to_symbol('quote')
        return Pair(val, Pair(scm_read(), NULL))
    elif syntax == "(":
        return read_tail()
    else:
