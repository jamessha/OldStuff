...
int main( int argc, char **argv )
{
    ...
    /* EDIT HOW MANY YOU WANT TO TEST HERE */
    int test = 35;
    
    /* Try different size */
    for ( int i = 0; i < test; ++i ){

        /* Testing randomly the first 1/3 */
        if (i < test / 3) {
            if (i == 0)
                printf("\nTesting randomly:\n");
            m = 1000 + (rand() % 9001);
        ...
        /* Finally, test for prime */
        else { // the last test cases
            if (i == 2 * test / 3)
    ...
    if (average_random >= 50){
        ...
        total = 55;
        int ec = 0;
        for (i = 52; i < average_random; i += 2)
            ec++;
	printf("%d/55\n", total);
        printf("Potential extra credit: %d", ec);
    }
