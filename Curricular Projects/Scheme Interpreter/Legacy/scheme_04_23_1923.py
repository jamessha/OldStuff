...
class Evaluation:
    ...
    def do_and_form(self):
        ...
        def checker(exp):
        	if exp.cdr == NULL:
        		if self.full_eval(exp.car) == TRUE:
        			return True
        		elif self.full_eval(exp.car) == FALSE:
        			return False
        		else:
        			return self.full_eval(exp.car)
        	elif self.full_eval(exp.car) == FALSE:
        		return False
        	return checker(exp.cdr)
        if checker(self.expr.cdr) == True:
        	self.set_expr(TRUE)
        elif checker(self.expr.cdr) == False:
        	self.set_expr(FALSE)
        else:
        	self.set_expr(checker(self.expr.cdr))
    ...
    def do_or_form(self):
        ...
        if self.expr.length() == 1:
            ...
            return
        self.expr = self.expr.cdr
        def checker(exp):
        	if self.full_eval(exp.car):
        		return self.full_eval(exp.car)
        	elif exp.length() == 1:
        		return False
        	return checker(exp.cdr)
        if checker(self.expr) == False:
        	self.set_expr(FALSE)
        else:
        	self.set_expr(checker(self.expr))
