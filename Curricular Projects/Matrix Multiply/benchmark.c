...
int main(int argc, char **argv) {
    ...
    int m_values[3] = {1000, 7919, 10000};
    int n_values[3] = {32, 91, 100};

    /* Try different m */
    //for( int m = 32; m < 33; m = m+1+m/3 )
    for (int n_index = 0; n_index < sizeof(n_values)/sizeof(int); n_index++) {
        int n = n_values[n_index];
        for (int m_index = 0; m_index < sizeof(m_values)/sizeof(int); m_index++) {
