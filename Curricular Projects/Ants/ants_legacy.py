...
class Ant(Insect):
    ...
    def can_contain(self, other):
    	if self.container == True and other.container == False and self.has_ant == False:
    		self.has_ant = True
    		return True
    	else:
    		return False
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
    	if self.place.bees:
    		bees2 = self.place.bees[:]
    		fire_ant_place = self.place
    		Insect.reduce_armor(self, amount)
    		if self.armor <= 0:
    			for bee in bees2:
    				if fire_ant_place.bees[fire_ant_place.bees.index(bee)] is not None:
    					fire_ant_place.bees[fire_ant_place.bees.index(bee)].reduce_armor(FireAnt.fire_damage)
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        """
        le_place = self.place
        max_place = self.place
        i = 0
        while i <= self.max_range:
        	if max_place.entrance != hive:
        		max_place = max_place.entrance
        	i += 1
        while le_place != max_place:
        	if le_place.bees:
        		return random_or_none(le_place.bees)
        	else:
        		le_place = le_place.entrance
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
    	if self.place.bees:
    		bees2 = self.place.bees[:]
    		for bee in bees2:
    			if self.place.bees[self.place.bees.index(bee)] is not None:
...
def apply_effect(effect, bee, duration):
    ...
    global old_actions, original_action
    if not original_action:
    	original_action = bee.action
    if duration >= 1:
    	old_actions.append(bee.action)
    def new_method(colony):
    	nonlocal duration, effect, bee
    	global old_action
    	if duration > 0:
    		effect(original_action)(colony)
    	else:
    		if old_actions:
    			old_actions[-1](colony)
    			bee.action = old_actions[-1]
    			old_actions.remove(old_actions[-1])
    		else:
