...
class Evaluation:
    ...
    def step(self):
        ...
        expr = self.expr
        if expr.symbolp():
            global _PRIMITIVES_LIST
            if len(_PRIMITIVES_LIST) != len(_PRIMITIVES):
            	for key, func in _PRIMITIVES:
            		if key not in _PRIMITIVES_LIST:
            			_PRIMITIVES_LIST.append(key)
            if str(expr) in _PRIMITIVES_LIST:
            	for key, func in _PRIMITIVES:
            		if key == str(expr):
            			self.set_value(PrimitiveFunction(func))
            else:
    ...
    def do_lambda_form(self):
        ...
        func = LambdaFunction(self.expr.cdr.car, self.expr.cdr.cdr, self.env)
        self.set_value(func)
